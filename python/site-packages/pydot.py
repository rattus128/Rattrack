...
class Graph(Common):
    ...
    def del_edge(self, src_or_list, dst=None, index=None):
        ...
        if isinstance(src_or_list, Edge):
            src = src_or_list.get_source()
            dst = src_or_list.get_destination()
        elif isinstance( src_or_list, (list, tuple)):
        ...
        if (src, dst) in self.obj_dict['edges']:
            ...
            if isinstance(src_or_list, Edge):
                for obj_dict in self.obj_dict['edges'][(src, dst)]:
                    if (obj_dict["sequence"] == src_or_list.get_sequence()):
                        self.obj_dict['edges'][(src, dst)].remove(obj_dict)
                return True
